"""WebSocket API for ClimateMind room CRUD operations."""
...
def _get_coordinator(hass: HomeAssistant) -> ClimateMindCoordinator | None:
    """Return the ClimateMindCoordinator from hass.data, or None."""
    coordinator: ClimateMindCoordinator | None = hass.data.get(DOMAIN, {}).get("coordinator")
    return coordinator
...
def _validate_no_own_entities(config: dict, own_prefix: str) -> str | None:
    """Check that no ClimateMind-owned entities are assigned. Returns error message or None."""
    for field in ("thermostats", "acs", "window_sensors", "covers", "occupancy_sensors"):
        for eid in config.get(field, []):
            if eid.split(".", 1)[-1].startswith(own_prefix):
                return f"Cannot assign ClimateMind's own entity '{eid}' to a room"
...
@websocket_api.websocket_command({vol.Required("type"): "climatemind/rooms/list"})
...
async def websocket_save_room(
    ...
    # Reject ClimateMind's own entities to prevent self-assignment (#86)
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "climatemind/rooms/delete",
        vol.Required("area_id"): str,
    }
)
@websocket_api.async_response
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "climatemind/override/set",
        vol.Required("area_id"): str,
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "climatemind/override/clear",
        vol.Required("area_id"): str,
    }
)
@websocket_api.async_response
...
@websocket_api.websocket_command({vol.Required("type"): "climatemind/settings/get"})
@websocket_api.async_response
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "climatemind/settings/save",
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "climatemind/analytics/get",
        vol.Required("area_id"): str,
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "climatemind/thermal/reset",
        vol.Required("area_id"): str,
    }
)
@websocket_api.async_response
...
@websocket_api.websocket_command({vol.Required("type"): "climatemind/thermal/reset_all"})
@websocket_api.async_response
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "climatemind/model/boost_learning",
        vol.Required("area_id"): str,
    }
)
@websocket_api.async_response
...
@websocket_api.websocket_command({vol.Required("type"): "climatemind/diagnostics/get"})
@websocket_api.async_response
...
@websocket_api.websocket_command(
    {
        vol.Required("type"): "climatemind/covers/clear_override",
        vol.Required("area_id"): str,
    }
)
@websocket_api.async_response
...
def async_register_websocket_commands(hass: HomeAssistant) -> None:
    """Register all ClimateMind WebSocket commands."""
