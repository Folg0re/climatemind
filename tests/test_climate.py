"""Tests for ClimateMind climate platform."""
...
from custom_components.climatemind.climate import (
    ClimateMindOverrideClimate,
...
def test_create_room_climates(mock_coordinator):
    ...
    assert isinstance(climates[0], ClimateMindOverrideClimate)


def test_unique_id_and_entity_id(mock_coordinator):
    """Climate entity has correct unique_id and entity_id."""
    coordinator, _ = mock_coordinator
    entity = ClimateMindOverrideClimate(coordinator, "living_room")
    assert entity.unique_id == "climatemind_living_room_override"
    assert entity.entity_id == "climate.climatemind_living_room_override"
...
def test_hvac_mode_off_when_room_missing(mock_coordinator):
    ...
    entity = ClimateMindOverrideClimate(coordinator, "nonexistent")
    assert entity.hvac_mode == HVACMode.OFF
...
async def test_async_setup_entry_creates_entities_for_all_rooms():
    ...
    assert all(isinstance(e, ClimateMindOverrideClimate) for e in entities)
