...
@customElement("climatemind-panel")
export class ClimateMindPanel extends LitElement {
  ...
  private async _loadRooms() {
    ...
    try {
      ...
      console.debug("[ClimateMind] loadRooms:", err);
    } finally {
  ...
  private async _onDeleteRoom() {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.debug("[ClimateMind] deleteRoom:", err);
    }
  }
  ...
  private async _onHideRoom(e: CustomEvent<{ areaId: string }>) {
    ...
    try {
      await this.hass.callWS({ type: "climatemind/settings/save", hidden_rooms: newHidden });
    } catch (err) {
      // eslint-disable-next-line no-console
      console.debug("[ClimateMind] hideRoom:", err);
    }
  }
  ...
  private async _unhideRoom(areaId: string) {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.debug("[ClimateMind] unhideRoom:", err);
    }
  }
  ...
  private async _saveRoomOrder(order: string[]) {
    ...
    try {
      await this.hass.callWS({ type: "climatemind/settings/save", room_order: order });
    } catch (err) {
      // eslint-disable-next-line no-console
      console.debug("[ClimateMind] saveRoomOrder:", err);
    }
  }
  ...
  private _navigate(path: string) {
    history.replaceState(null, "", `/climatemind${path}`);
...
declare global {
  interface HTMLElementTagNameMap {
    "climatemind-panel": ClimateMindPanel;
  }
}
